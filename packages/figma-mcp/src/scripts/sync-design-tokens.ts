...
interface SyncOptions {
  fileKey: string;
  outputPath: string;
  format: 'json' | 'css' | 'scss' | 'ts';
  validate: boolean;
  dryRun: boolean;
}

async function syncDesignTokens(options: SyncOptions): Promise<void> {
...
function generateCSSVariables(tokenGroups: any[]): string {
  let css = ':root {\n';
...
function generateSCSSVariables(tokenGroups: any[]): string {
  let scss = '';
...
function generateTypeScriptTokens(tokenGroups: any[]): string {
