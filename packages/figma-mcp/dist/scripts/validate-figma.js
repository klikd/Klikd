...
config();
async function validateFigmaFile(options) {
    ...
    try {
        ...
        if (options.detailed) {
            ...
            if (options.checkStructure) {
                ...
                const hasColorStyles = Object.values(file.styles).some((style) => style.styleType === 'FILL' || style.styleType === 'TEXT');
                console.log(`  Color Styles: ${hasColorStyles ? '✅' : '❌'}`);
                // Check for text styles
                const hasTextStyles = Object.values(file.styles).some((style) => style.styleType === 'TEXT');
                console.log(`  Text Styles: ${hasTextStyles ? '✅' : '❌'}`);
                // Check for component variants
                const hasVariants = Object.values(file.components).some((component) => component.componentProperties && Object.keys(component.componentProperties).length > 0);
...
//# sourceMappingURL=validate-figma.js.map